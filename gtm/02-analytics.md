...
(GitHub's API has no anonymous write path), which means shipping a secret in client-side JS —
exactly the kind of "no key" promise tripkit's whole pitch rests on breaking. The alternative,
